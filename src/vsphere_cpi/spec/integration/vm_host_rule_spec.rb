...
describe 'Host Groups in Cluster and VM Host Rules' do
  context 'when host groups are defined in AZ' do
    ...
    let(:first_host_vm_group_name) { @first_host_group+'MUST'+VSphereCloud::Resources::Cluster::CLUSTER_VM_GROUP_SUFFIX }
    let(:second_host_vm_group_name) { @second_host_group+'MUST'+VSphereCloud::Resources::Cluster::CLUSTER_VM_GROUP_SUFFIX }
    let(:third_host_vm_group_name) { @third_host_group+'MUST'+VSphereCloud::Resources::Cluster::CLUSTER_VM_GROUP_SUFFIX }
    let(:first_host_should_vm_group_name) { @first_host_group+'SHOULD'+VSphereCloud::Resources::Cluster::CLUSTER_VM_GROUP_SUFFIX }
    let(:second_host_should_vm_group_name) { @second_host_group+'SHOULD'+VSphereCloud::Resources::Cluster::CLUSTER_VM_GROUP_SUFFIX }
    let(:third_host_should_vm_group_name) { @third_host_group+'SHOULD'+VSphereCloud::Resources::Cluster::CLUSTER_VM_GROUP_SUFFIX }
      ...
      context 'and one host_group is specified in each cluster' do
        ...
        it 'creates one VM on one of the hosts in the host groups of any cluster' do
          simple_vm_lifecycle(cpi, '', vm_type, get_network_spec) do |vm_id|
            ...
            expect(vm_host_rule.vm_group_name).to eq(expected_host_group_mob.name+default_drs_rule+VSphereCloud::Resources::Cluster::CLUSTER_VM_GROUP_SUFFIX)
        ...
        let(:options) do
          cpi_options(
            datacenters: [{
              ...
              'persistent_datastore_pattern' => @datastore_shared_pattern,
              clusters: [
                { @cluster_name => { host_group: {'name' => @second_host_group, 'drs_group' => 'Must'} } },
